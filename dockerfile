...
RUN mkdir -p /var/log \
    ...
    # Final: run every 15 minutes for testing purposes
    && echo "*/15 * * * * root cd /app && /usr/local/bin/node /app/node_modules/.bin/ts-node main.ts >> /var/log/cron.log 2>&1" >> /etc/cron.d/crawling-job \
