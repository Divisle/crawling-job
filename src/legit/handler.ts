...
import { LegitJobRepository } from "./database";
import { JobMessageData, buildJobMessage } from "../template";
import axios from "axios";
import { buildMessage } from "../global";

export class LegitJobHandler {
  constructor(private db = new LegitJobRepository(new PrismaClient())) {
  ...
  async scrapeJobs(): Promise<Prisma.LegitJobCreateInput[]> {
    try {
      const response: {
        data: {
          name: string;
      ...
      // console.log(`Scraped ${data.length} jobs from Legit`);
      // console.log(data);
      return data;
    } catch (error) {
  ...
  static async run() {
    const handler = new LegitJobHandler();
...
// LegitJobHandler.run().then(async (res) => {
