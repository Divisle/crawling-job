...
export class AbnormalJobHandler {
  ...
  async scrapeJobs(): Promise<Prisma.AbnormalJobCreateInput[]> {
    try {
      ...
      const data: Prisma.AbnormalJobCreateInput[] = response.data.jobs.map(
        (job) => ({
          title: job.title,
      ...
      // console.log(`Scraped ${data.length} jobs from Abnormal`);
      // console.log(data);
      return data;
    } catch (error) {
  ...
  async sendMessage(data: JobMessageData[]) {
    const blocks = buildJobMessage(data, "Abnormal", "https://abnormal.ai", 1);
    return {
      blocks,
      channel: 1,
    };
  ...
  static async run() {
    const handler = new AbnormalJobHandler();
...
// AbnormalJobHandler.run().then((res) => {
