...
import { ZestJobRepository } from "./database";
import { buildJobMessage, JobMessageData } from "../template";
import { buildMessage } from "../global";

export class ZestScraper {
  private driver: WebDriver;

  constructor(private db = new ZestJobRepository(new PrismaClient())) {
  ...
  async scrapeJobs(): Promise<Prisma.ZestJobCreateInput[]> {
    await this.driver.get("https://www.zestsecurity.io/careers");

    // Additional wait to ensure all content is loaded
    await this.driver.sleep(2000);
    const jobElements = await this.driver.findElements(
      By.xpath(".//div[@class='careers_wrapper w-dyn-items']//a")
    );
    let jobData: Prisma.ZestJobCreateInput[] = [];
    for (let i = 0; i < jobElements.length; i++) {
      try {
        ...
        const title = await jobElement
          .findElement(
            By.xpath(".//div[@class='cad-job_title']//div[not(@class)]")
          )
          .getText();
        const location = await jobElement
          .findElement(
            By.xpath(
              ".//div[@class='card-job_location-wrapper']//div[not(@class)]//div[@class='u-flex cc-gap-xs u-font-bold']"
            )
          )
          .getText();
    ...
    // console.log(`Scraped ${jobData.length} jobs from Zest.`);
    // console.log(jobData);
    return jobData;
  }

  async filterData(
    jobData: Prisma.ZestJobCreateInput[]
