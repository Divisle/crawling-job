...
export function buildLegionJobMessage(data: {
  ...
  blocks.push({
    ...
    text: {
      type: "mrkdwn",
      text: `We found *${data.newJobs.length} new jobs*, *${data.updateJobs.length} updated jobs* and *${data.deleteJobs.length} jobs removed* from <https://www.legionsecurity.ai/|Legion Security>`,
    },
  });
  if (
